...
def generate_mean_std(n, agent, new, end_loc, box_loc):
    if new: 
        ...
        severe = set(load("nav_exp_15_15_s"))
        mild = set(load("nav_exp_15_15_m"))
        no_nse = set(load("nav_exp_15_15_nn"))
    ...
    print(len(severe), len(mild), len(no_nse))
    # print(severe)
    save("nav_exp_15_15_s", list(severe))
    save("nav_exp_15_15_m", list(mild))
    save("nav_exp_15_15_nn", list(no_nse))
...
class Agent:
    ...
    def getAction(self, state):
        x = random.random()
        try:
            a = np.random.choice(self.pi[state], p = self.prob2[state])
        except:
            # print(state, np.sum(self.prob2[state]))
            a = np.random.choice(self.pi[state], p = self.prob1[state])
